...
def render(args):
    ...
    with torch.no_grad():
        ...
        num_frames = 60
        video_path = 'output'
        Ts = define_camera_path(num_frames, 0., 0., 0., path_type='straight-line', return_t_only=True)
        crop = 32
        kernel = torch.ones(5, 5, device=device)

        frames = []
        for t_step, t in tqdm(enumerate(range(num_frames)), total=num_frames, ncols=150, desc='generating video'):
            t_step = torch.tensor([t_step]).to(device)
            time = t / (num_frames - 1)
            flow_f = renderer.euler_integration(flow, t_step)
            flow_b = renderer.euler_integration(-flow, num_frames - 1 - t_step)
